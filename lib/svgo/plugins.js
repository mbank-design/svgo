 ...
 */
const invokePlugins = (ast, info, plugins, overrides, globalOverrides, validateResult = {}) => {
  for (const plugin of plugins) {
    ...
    if (plugin.type === 'validate') {
      validate(ast, info, plugin, validateResult);
    }
    if (plugin.type === 'full') {
      if (plugin.active) {
 ...
 * @param {Object} info extra information
 * @param {Array} plugin plugins list to process
 ...
 * @return {Object} output data
 */
function validate(data, info, plugin, validateResult) {
// function validate(data, info, plugins, validateResult) {
  // plugins.forEach(function (plugin) {
  //   if (plugin.active) {
  //     validateResult = plugin.fn(data, validateResult, plugin.params, info);
    // }
  // });
      validateResult = plugin.fn(data, validateResult, plugin.params, info);
