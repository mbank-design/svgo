...
module.exports = function makeProgram(program) {
  program
    ...
    .option(
      '--asset-type <STRING>',
      'Input one of the following asset types (ICON_REGULAR, ICON_COLOR, ILLUSTRATION)'
    )
    .option(
      '-o, --output <OUTPUT...>',
...
async function action(args, opts, command) {
  ...
  if (input.length !== 0) {
    ...
    if (input[0] === '-') {
      ...
      if (opts.validate != null && opts.assetType && input.length === 1) {
        const assetTypes = ['ICON_REGULAR', 'ICON_COLOR', 'ILLUSTRATION'];
        ...
        (opts.validate == null && opts.assetType) ||
        (opts.validate != null && !opts.assetType)
      ) {
        console.error(
        ...
        process.exit(1);
      } else if (input.length != 1 && opts.validate != null) {
