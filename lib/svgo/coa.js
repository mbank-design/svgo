...
module.exports = function makeProgram(program) {
  program
    ...
    .option(
      ...
      'Indicate that directory, will be under validation'
    )
    .option('-val, --validate', 'Input SVG file to validate')
    .option(
      '-at, --asset-type <STRING>',
      'Set correct asset type for validated asset'
    )
    .option(
      '-o, --output <OUTPUT...>',
  ...
  if (input.length !== 0) {
    ...
    if (input[0] === '-') {
      ...
      if (
        ...
        if (!assetTypes.includes(opts.assetType)) {
          console.log(
            `error: option '--asset-type' takes only following params ${assetTypes}`
          );
          process.exit(1);
        }
        ...
        await Promise.all(
          input.map((file, index) =>
            validateFile(config, file, output[index], opts.assetType)
          )
        );
      } else if (
        opts.validate !== undefined &&
        ...
        const directoryInput = input[0];

        if (!config.quiet) {
          console.log(`Processing directory '${directoryInput}':\n`);
        }
        return FS.promises.readdir(directoryInput).then((files) => {
          validateDirectory(
            config,
            directoryInput,
            files,
        ...
        (opts.validate === undefined && opts.assetType) ||
        (opts.assetType === undefined && opts.validate)
      ) {
        console.error(
          "error: to use validation feature u need to provide both '--validation' flag and '--asset-type' with correct value"
        );
        process.exit(1);
      } else if (input.length !== 1 && opts.assetType) {
        console.error(
          'error: provide exactly one source to validate or indicate a directory'
        );
        process.exit(1);
      } else {
        await Promise.all(
...
/**
 * Validate SVG files in a directory.
 * @param {Object} config options
 * @param {string} dir input directory
 * @param {string} files files in directory
 * @param {string} output output directory
 * @param {string} assetType asset type to validate
 * @return {Promise}
 */
function validateDirectory(config, dir, files, output, assetType) {
