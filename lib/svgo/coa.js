...
module.exports = function makeProgram(program) {
  program
    ...
    .option('--validate', 'Input SVG file to validate')
    .option(
      '--asset-type <STRING>',
      'Input one of the following asset types (ICON_REGULAR, ICON_COLOR, LOGO, ILLUSTRATION, FLAG, AVATAR, ANIMATION)'
    )
    .option(
      '-o, --output <OUTPUT...>',
...
async function action(args, opts, command) {
  ...
  if (input.length !== 0) {
    ...
    if (input[0] === '-') {
      ...
      if (opts.validate !== undefined && opts.assetType && input.length === 1) {
        const assetTypes = [
          'ICON_REGULAR',
          'ICON_COLOR',
          'LOGO',
        ...
        console.error('error: provide exactly one SVG file to validate');
        process.exit(1);
      } else {
        await Promise.all(
          input.map((file, index) => optimizeFile(config, file, output[index]))
        );
      }
    }
...
function optimizeFile(config, file, output) {
  return FS.promises.readFile(file, 'utf8').then(
    ...
    (error) => {
      checkOptimizeFileError(config, file, output, error);
    }
  );
}
