...
const {
  ...
  validate: validateAgnostic,
  createContentItem,
} = require('./svgo.js');
const { parseSvg: parseSvgAgnostic } = require('./parser.js');
...
const validate = (input, filename, type, config) => {
  ...
  return validateAgnostic(input, filename, type, {
    ...config,
  });
};
...
const parseSvg = (data, from) => {
  if (from == null) {
    ...
    throw Error('From should be an string');
  }

  return parseSvgAgnostic(data, from);
};
